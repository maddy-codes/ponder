...
const normalise = (e: any) => ({
  ...
  guardrails: (e.guardrails ?? []).map((g: any) => String(g)),
  guardrailVerdicts: (e.guardrail_verdicts ?? []).map((v: any) => ({
    name: String(v.name ?? ""),
    outcome: String(v.outcome ?? "allow"),
    detail: String(v.detail ?? ""),
  })),
  guardrailEscalated: Boolean(e.guardrail_escalated ?? false),
  guardrailBlocked: Boolean(e.guardrail_blocked ?? false),
